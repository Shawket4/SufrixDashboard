...
# Task
Your task is to write extremely intensive tests for the `dashboard` page module in `src/pages/dashboard/`.

# Context to Gather First
Before writing any code, deep dive into the source code of the `dashboard` page. Read the main page components, the hooks fetching data, and any forms. Pay special attention to:
...
# Execution Loop
1. Create `src/pages/dashboard/__tests__/dashboard.test.tsx`.
2. Write the tests covering ALL edge cases.
3. Run the specific test: `npx vitest run src/pages/dashboard`.
