...
# Task
Your task is to write extremely intensive tests for the `orders` page module in `src/pages/orders/`.

# Context to Gather First
Before writing any code, deep dive into the source code of the `orders` page. Read the main page components, the hooks fetching data, and any forms. Pay special attention to:
...
# Execution Loop
1. Create `src/pages/orders/__tests__/orders.test.tsx`.
2. Write the tests covering ALL edge cases.
3. Run the specific test: `npx vitest run src/pages/orders`.
