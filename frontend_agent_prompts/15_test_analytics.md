...
# Task
Your task is to write extremely intensive tests for the `analytics` page module in `src/pages/analytics/`.

# Context to Gather First
Before writing any code, deep dive into the source code of the `analytics` page. Read the main page components, the hooks fetching data, and any forms. Pay special attention to:
...
# Execution Loop
1. Create `src/pages/analytics/__tests__/analytics.test.tsx`.
2. Write the tests covering ALL edge cases.
3. Run the specific test: `npx vitest run src/pages/analytics`.
