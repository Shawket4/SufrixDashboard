...
# Task
Your task is to write extremely intensive tests for the `discounts` page module in `src/pages/discounts/`.

# Context to Gather First
Before writing any code, deep dive into the source code of the `discounts` page. Read the main page components, the hooks fetching data, and any forms. Pay special attention to:
...
# Execution Loop
1. Create `src/pages/discounts/__tests__/discounts.test.tsx`.
2. Write the tests covering ALL edge cases.
3. Run the specific test: `npx vitest run src/pages/discounts`.
