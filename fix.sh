...
#
#  Scope: Analytics.tsx ONLY — full clean rewrite to restore broken charts.
#  Everything else (Orders, Discounts, types, App, Sidebar) was already
#  applied by patch_react.sh and is left untouched.
#
#  Run from: ~/Desktop/RueDashboard
# =============================================================================
set -e

echo "=== Rue POS — React patch v2: Analytics fix ==="
echo "Working in: $(pwd)"
echo ""

# Guard: must be run from the project root
if [ ! -f "src/App.tsx" ]; then
  echo "ERROR: Run this from ~/Desktop/RueDashboard"
  exit 1
fi
...
# ─────────────────────────────────────────────────────────────────────────────
#  Analytics.tsx — full clean rewrite
#  Reads from the exact same backend endpoints that were always there:
...
#    GET /reports/orgs/:id/comparison
...
echo "Committing..."
git add .
git commit -m "fix: Analytics.tsx full clean rewrite — restore revenue charts (item 4)"

echo ""
echo "==================================================="
echo "  Done. Analytics restored:"
echo "  - Overview: revenue stats + payment pie + top items"
echo "  - Revenue: area chart by payment method, orders bar, discounts bar"
echo "  - Items: full table + categories + addons"
echo "  - Tellers: horizontal bar + detail table"
echo "  - Branches: stacked bar + comparison table"
echo "  - Inventory: stock levels with low-stock badges"
echo "==================================================="